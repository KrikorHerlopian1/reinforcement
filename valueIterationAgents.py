...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def maxQ(self, st):
        ...
        q_s = (self.computeQValueFromValues(st, i) for i in actions)
        return max(q_s, default=0)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        def rewardIf(s, a, s_):
            j = self.discount*self.getValue(s_)
            i = self.mdp.getReward(s, a, s_)
            return i + j

        statesAndP = self.mdp.getTransitionStatesAndProbs(state, action)
        return sum(p1 * rewardIf(state, action, s1) for s1, p1 in statesAndP)
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def getStatePredecessors(self):
        ...
        for s in self.mdp.getStates():
            for pa in self.mdp.getPossibleActions(s):
                for s1, z in self.mdp.getTransitionStatesAndProbs(s, pa):
                    predecessors[s1].append(s)
        return predecessors
    ...
    def runValueIteration(self):
        ...
        for s in filter(isNotTerminal, self.mdp.getStates()):
            difference = abs(self.values[s] - self.maxQ(s))
            que.push(s, -difference)
        ...
        for i in range(self.iterations):
            ...
            for p in predecessors[s]:
                difference = abs(self.values[p] - self.maxQ(p))
                if difference > self.theta:
                    que.update(p, -difference)
